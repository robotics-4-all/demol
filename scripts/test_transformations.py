...
from demol.transformations import m2t_rpi, m2t_riot, m2m_smauto, demol_to_json, m2t_docs
import json
...
class TransformationTester:
    ...
    def test_transformation(self, model_path: Path, platform: str, output_dir: Path) -> TransformationTestResult:
        """
    ...
    def run_tests(self, test_dir: Path, platform: str, output_base: Path) -> List[TransformationTestResult]:
        """
        ...
        if self.use_rich:
            with Progress(
                ...
                task = progress.add_task(f"[cyan]Testing {platform} generation...", total=len(model_files))
    ...
    def create_results_table(self, results: List[TransformationTestResult]) -> "Table":
        ...
        for result in results:
            ...
            table.add_row(result.model_path, result.platform, str(len(result.files_generated)), status)
