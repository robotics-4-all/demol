...
class InfrastructureSvgGenerator(BaseCodeGenerator):
    ...
    def generate(self) -> None:
        ...
        for i, conn in enumerate(connections):
            ...
            peripherals.append({"name": p_name, "type": p_ref.type, "x": x_periph, "y": p_y, "protocol": protocol})
        ...
        for i, app_name in enumerate(apps_list):
            ...
            apps.append({"name": app_name, "x": x_apps, "y": a_y})
        ...
        for i, conn in enumerate(connections):
            ...
            topics.append({"text": topic, "y": topics_y + i * 12})
        ...
        template_data = {
            ...
            "box": {"width": self.BOX_WIDTH, "height": self.BOX_HEIGHT},
            "layers": [
                {"name": "Edge Layer", "x": x_board},
                {"name": "Communication Layer", "x": x_broker},
                {"name": "Application Layer", "x": x_apps},
            ],
            "separators": [{"x": (x_board + x_broker) / 2 - 50}, {"x": (x_broker + x_apps) / 2 - 50}],
            "peripherals": peripherals,
            "board": {"name": board.name, "type": board.type, "x": x_board, "y_mid": y_mid},
            "broker": {"host": broker["host"], "port": broker["port"], "x": x_broker, "y_mid": y_mid},
            "apps": apps,
