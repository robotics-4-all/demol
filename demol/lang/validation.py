...
class ValidationReporter:
    ...
    def __init__(self, use_rich: bool = True):
        ...
        self.console: Optional["_ConsoleType"] = Console() if self.use_rich else None
    ...
    def print_summary_panel(self, results: List[ValidationResult]) -> None:
        ...
        if failed > 0 and self.use_rich and self.console:
            self.console.print()
        ...
        for file_path in file_paths:
            ...
            if show_progress:
                status_symbol = (
                    "✓"
                    if result.status == ValidationStatus.PASS
                    else ("⚠" if result.status == ValidationStatus.WARN else "✗")
                )
