        ...
        for prop in connection.props:
            ...
            if prop.name == "bus_speed":
                if not isinstance(prop.value, int) or prop.value <= 0:
                    raise_validation_error(
                        connection,
                        "[Conn-I2C] Property 'bus_speed' must be a positive integer.",
                        "InvalidValueError",
                    )
    ...
    def validate(
        ...
        for prop in connection.props:
            if prop.name == "name":
                raise_validation_error(
                    connection,
                    "[Conn-SPI] Property 'name' is deprecated for SPI connections. "
                    "Use 'bus_speed' or 'mode' instead.",
                    "DeprecatedPropertyError",
                )
            if prop.name not in valid_props:
                raise_validation_error(
            ...
            if prop.name == "bus_speed":
                if not isinstance(prop.value, int) or prop.value <= 0:
                    raise_validation_error(
                        connection,
                        "[Conn-SPI] Property 'bus_speed' must be a positive integer.",
                        "InvalidValueError",
                    )
            elif prop.name == "mode":
                if not isinstance(prop.value, int) or prop.value not in [0, 1, 2, 3]:
                    raise_validation_error(
                        connection,
                        "[Conn-SPI] Property 'mode' must be an integer between 0 and 3.",
                        "InvalidValueError",
                    )
        ...
        for pin_type in spi_pin_types:
            ...
            if not any(pin_type in str(f).lower() for f in board_funcs):
                raise_validation_error(
                    connection,
                    f"[Conn-SPI] Board pin {getattr(board_pin, 'name', board_pin)} does not have {pin_type.upper()} (SPI) functionality",
                    "SPIFunctionError",
                )
            ...
            if not any(pin_type in str(f).lower() for f in peripheral_funcs):
                raise_validation_error(
                    connection,
                    f"[Conn-SPI] Peripheral pin {getattr(peripheral_pin, 'name', peripheral_pin)} does not have {pin_type.upper()} (SPI) functionality",
                    "SPIFunctionError",
                )
        ...
        for prop in connection.props:
            ...
            if prop.name == "baudrate":
                ...
                if isinstance(prop.value, (int, float)):
                    ...
                    raise_validation_error(
                        connection,
                        "[Conn-UART] Property 'baudrate' must be a positive integer.",
                        "InvalidValueError",
                    )
        ...
        for prop in connection.props:
            ...
            if prop.name == "frequency":
                if not isinstance(prop.value, (int, float)) or prop.value <= 0:
                    raise_validation_error(
                        connection,
                        "[Conn-PWM] Property 'frequency' must be a positive number.",
                        "InvalidValueError",
                    )
                ...
                if not isinstance(prop.value, int) or prop.value < 0:
                    raise_validation_error(
                        connection,
                        "[Conn-PWM] Property 'channel' must be a non-negative integer.",
                        "InvalidValueError",
                    )
