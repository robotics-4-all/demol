...
def get_connection_endpoints(connection):
    ...
    if hasattr(connection, "_from_ref"):
        return (connection._from_ref, connection._from_name, connection._to_ref, connection._to_name)
...
def validate_power_connection(board_pin, peripheral_pin, connection) -> None:
    ...
    if board_voltage is None:
        raise_validation_error(connection, f"Invalid board power pin type: {board_pin.ptype}", "PowerTypeError")

    if peripheral_voltage is None:
        raise_validation_error(
            connection, f"Invalid peripheral power pin type: {peripheral_pin.ptype}", "PowerTypeError"
        )
    ...
    if board_voltage != 0.0 and peripheral_voltage != 0.0:
        if not are_voltages_compatible(board_voltage, peripheral_voltage):
            raise_validation_error(
                ...
                "Voltage difference exceeds 0.5V tolerance.",
...
def validate_gpio_connection(board_pin, peripheral_pin, connection) -> None:
    ...
    for prop in connection.props:
        if prop.name == "name":
            raise_validation_error(
                connection,
                "[Conn-GPIO] Property 'name' is deprecated for GPIO connections. "
                "Use 'mode', 'pullup', or 'pulldown' instead.",
        ...
        if prop.name == "mode":
            ...
            if not isinstance(prop.value, bool):
                raise_validation_error(
                    connection, f"[Conn-GPIO] Property '{prop.name}' must be a boolean.", "InvalidTypeError"
                )


def validate_i2c_connection(board_sda, board_scl, peripheral_sda, peripheral_scl, slave_addr: int, connection) -> None:
    """
    ...
    for prop in connection.props:
        if prop.name == "name":
            raise_validation_error(
                connection,
                "[Conn-I2C] Property 'name' is deprecated for I2C connections. "
                "Use 'slave_address' or 'bus_speed' instead.",
                "DeprecatedPropertyError",
            )
        if prop.name not in valid_props:
            raise_validation_error(
        ...
        if prop.name == "bus_speed":
            if not isinstance(prop.value, int) or prop.value <= 0:
                raise_validation_error(
                    connection, "[Conn-I2C] Property 'bus_speed' must be a positive integer.", "InvalidValueError"
                )
...
def validate_spi_connection(board_pins: Dict[str, object], peripheral_pins: Dict[str, object], connection) -> None:
    """
    ...
    for prop in connection.props:
        if prop.name == "name":
            raise_validation_error(
                connection,
                "[Conn-SPI] Property 'name' is deprecated for SPI connections. " "Use 'bus_speed' or 'mode' instead.",
                "DeprecatedPropertyError",
            )
        if prop.name not in valid_props:
            raise_validation_error(
        ...
        if prop.name == "bus_speed":
            if not isinstance(prop.value, int) or prop.value <= 0:
                raise_validation_error(
                    connection, "[Conn-SPI] Property 'bus_speed' must be a positive integer.", "InvalidValueError"
                )
        elif prop.name == "mode":
            if not isinstance(prop.value, int) or prop.value not in [0, 1, 2, 3]:
                raise_validation_error(
                    connection, "[Conn-SPI] Property 'mode' must be an integer between 0 and 3.", "InvalidValueError"
                )
...
def validate_uart_connection(board_tx, board_rx, peripheral_tx, peripheral_rx, baudrate, connection) -> None:
    """
    ...
    for prop in connection.props:
        if prop.name == "name":
            raise_validation_error(
                connection,
                "[Conn-UART] Property 'name' is deprecated for UART connections. "
                "Use 'baudrate', 'parity', 'stop_bits', or 'data_bits' instead.",
                "DeprecatedPropertyError",
            )
        if prop.name not in valid_props:
            raise_validation_error(
        ...
        if prop.name == "baudrate":
            ...
            if isinstance(prop.value, (int, float)):
                ...
                raise_validation_error(
                    connection, "[Conn-UART] Property 'baudrate' must be a positive integer.", "InvalidValueError"
                )
            ...
            if prop.value not in [1, 2]:
                raise_validation_error(
                    connection, f"[Conn-UART] Invalid stop_bits '{prop.value}'. Must be 1 or 2.", "InvalidValueError"
                )
    ...
    if not any("rx" in str(f).lower() for f in peripheral_tx_funcs):
        raise_validation_error(
            ...
            "UART requires connecting Board TX to Peripheral RX.",
            "UARTFunctionError",
        )
    ...
    if not any("tx" in str(f).lower() for f in peripheral_rx_funcs):
        raise_validation_error(
            ...
            "UART requires connecting Board RX to Peripheral TX.",
            "UARTFunctionError",
        )
...
def validate_pwm_connection(board_pin, peripheral_pin, connection) -> None:
    ...
    for prop in connection.props:
        ...
        if prop.name == "frequency":
            if not isinstance(prop.value, (int, float)) or prop.value <= 0:
                raise_validation_error(
                    connection, "[Conn-PWM] Property 'frequency' must be a positive number.", "InvalidValueError"
                )
            ...
            if not isinstance(prop.value, int) or prop.value < 0:
                raise_validation_error(
                    connection, "[Conn-PWM] Property 'channel' must be a non-negative integer.", "InvalidValueError"
                )
    ...
    if not any("pwm" in str(f).lower() for f in board_funcs):
        raise_validation_error(
            connection, f"[Conn-PWM] Board pin {board_pin.name} does not have PWM functionality", "PWMFunctionError"
        )
    ...
    if not peripheral_funcs:
        raise_validation_error(
            connection, f"[Conn-PWM] Peripheral pin {peripheral_pin.name} is not a valid IO pin", "PWMFunctionError"
        )
...
def validate_voltage_limits(model) -> None:
    ...
    for connection in model.connections:
        ...
        for pconn in connection.powerConns:
            ...
            board_pin = next((p for p in model.components.board.pins if p.name == board_pin_name), None)
...
def validate_io_voltage_compatibility(model) -> None:
    ...
    board_io_vcc_str = (
        board.operational.iovcc
        if hasattr(board.operational, "iovcc") and board.operational.iovcc
        else board.operational.vcc
    )
    ...
    for connection in model.connections:
        ...
        if (
            target_ref is None
            or not hasattr(target_ref, "operational")
            or not getattr(target_ref.operational, "iovcc", None)
        ):
            continue
    ...
    for connection in model.connections:
        ...
        if not hasattr(connection, "powerConns") or not connection.powerConns:
            raise_validation_warning(
                ...
                "connections to the board. Ensure proper grounding through external means "
                "or add a GND power connection for electrical safety.",
                "WF-Common-Ground",
            )
            continue
...
def validate_all_peripherals_connected(model) -> None:
    ...
    if unconnected_sources:
        raise_validation_warning(
            ...
            "Power sources should be connected to the board or a peripheral.",
...
def validate_connections(model) -> None:
    ...
    _board_pin_names = set(board_pins_map.keys())

    # Also collect power sources for pin lookup
    _power_sources_map = {ps.name: ps.ref for ps in model.components.powerSources}
        ...
        for data_conn in c.dataConns:
            ...
            for pin_map in data_conn.pins:
                ...
                if pin_map.fromPin not in data_source_pin_names:
                    raise_validation_error(
                        pin_map, f"Source pin '{pin_map.fromPin}' not found on '{data_source_name}'."
                    )
                    pins_valid = False
                # Check if toPin exists on sink component
                if pin_map.toPin not in data_sink_pin_names:
                    raise_validation_error(pin_map, f"Target pin '{pin_map.toPin}' not found on '{data_sink_name}'.")
                    pins_valid = False
            ...
            if conn_type == "gpio":
                ...
                validate_i2c_connection(source_sda, source_scl, sink_sda, sink_scl, slave_addr, data_conn)
                ...
                if is_from_board:
                    ...
                    validate_uart_connection(
                        pin_tx_source, pin_rx_source, pin_tx_sink, pin_rx_sink, baudrate, data_conn
                    )
                elif is_to_board:
                    # Source is Peripheral, Sink is Board
                    validate_uart_connection(
                        pin_tx_sink, pin_rx_sink, pin_tx_source, pin_rx_source, baudrate, data_conn
                    )
                else:
    ...
    for peripheral_def in model.components.peripherals:
        ...
        if name in peripheral_names:
            raise_validation_error(
                ...
                "Peripheral names must be unique within the device.",
